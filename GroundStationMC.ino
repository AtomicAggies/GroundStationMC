...
void loop()
{
  ...
  { // incoming avionics data
    ...
    {
      if (DEBUG)
        Serial.print("xbee msg not null: ");
      if (DEBUG)
      ...
      else if (strcmp(xbee_msg_token, "BMP") == 0)
      { // Incoming BMP Data: "BMP," + "Temperature," + "Pressure," + "Altitude," + "ENDDATA";
        Serial.println("Incoming BMP: temp,press,altitude");
        Serial.println(xbee_msg);
        Serial.println("_______________________");
      } // end if BMP
      else if (strcmp(xbee_msg_token, "BNO") == 0)
      { // Incoming BNO Data
        Serial.println("Incoming BNO: pitch,roll,yaw");
        Serial.println(xbee_msg);
        Serial.println("_______________________");
      } // end if BNO
      else if (strcmp(xbee_msg_token, "GPS") == 0)
      { // Incoming GPS Data
  ...
  if (digitalRead(ACTIVATE_PIN) == LOW && activation_state == ACTIVATED)
  { // physical activation lever is off. This implies we want to deactivate avionics.
    if (millis() - deac_data_timer > 500)
    {
  ...
  else if (digitalRead(ACTIVATE_PIN) == HIGH && activation_state == INACTIVE)
  { // lever is on. We want to activate avionics
